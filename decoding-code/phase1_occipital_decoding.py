...
all_sub_EEG_phase1_inf = {}
for file in os.listdir():
    mat_data = sio.loadmat(file)
    loaded_matrix = mat_data['eegKept']
    subID = re.findall(r'\d+', file)[0]
    all_sub_EEG_phase1_inf[subID] = loaded_matrix

os.chdir('../BehavioralDataAlignedTrainInfrequent')
all_sub_behav_phase1_inf = {}
for file in os.listdir():
    subID = re.findall(r'\d+', file)[0]
    all_sub_behav_phase1_inf[subID] = pd.read_csv(file)

os.chdir('../filteredEEGTrainFrequent')
all_sub_EEG_phase1_f = {}
for file in os.listdir():
    mat_data = sio.loadmat(file)
    loaded_matrix = mat_data['eegKept']
    subID = re.findall(r'\d+', file)[0]
    all_sub_EEG_phase1_f[subID] = loaded_matrix

os.chdir('../BehavioralDataAlignedTrainFrequent')
all_sub_behav_phase1_f = {}
for file in os.listdir():
    subID = re.findall(r'\d+', file)[0]
    all_sub_behav_phase1_f[subID] = pd.read_csv(file)
...
def process_single_subject(single_subject, eeg_f_data, eeg_inf_data, behav_f_data, behav_inf_data):
    ...
    np.save(f"phase1_acc_occ/model_accuracies_{single_subject}.npy", model_accuracies)
    np.save(f"phase1_acc_occ_f/model_accuracies_f_{single_subject}.npy", model_accuracies_f)
    np.save(f"phase1_acc_occ_inf/model_accuracies_inf_{single_subject}.npy", model_accuracies_inf)
...
if __name__ == "__main__":
    ...
    os.makedirs("phase1_acc_occ", exist_ok=True)
    os.makedirs("phase1_acc_occ_f", exist_ok=True)
    os.makedirs("phase1_acc_occ_inf", exist_ok=True)

    for single_subject in subjects:
        process_single_subject(single_subject, all_sub_EEG_phase1_f, all_sub_EEG_phase1_inf,
            all_sub_behav_phase1_f, all_sub_behav_phase1_inf)
